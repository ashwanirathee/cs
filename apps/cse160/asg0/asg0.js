...
function drawVector(v, color, name) {
  ...
  ctx.stroke();
  console.log(`Expected result with ${name} = (${v.elements[0]},${v.elements[1]})`);
}
...
function angleBetween(v1, v2) {
  ...
  let alpha = Math.round(Math.acos(cos_alpha) * (180 / Math.PI));
  return alpha;
}
...
function handleDrawEvent() {
  ...
  clearCanvas();
  // clearInput(); // should this be done?
  drawVector(vec1, "red", "v1");
  drawVector(vec2, "blue", "v2");
}
...
function handleDrawOperationEvent() {
  ...
  if (operation_type.value == "add") {
    ...
    drawVector(vec1, "red", "v1");
    drawVector(vec2, "blue", "v2");
    drawVector(vec1.add(vec2), "green", "v3");
    ...
    drawVector(vec1, "red", "v1");
    drawVector(vec2, "blue", "v2");
    drawVector(vec1.sub(vec2), "green", "v3");
    ...
    drawVector(vec1, "red", "v1");
    drawVector(vec2, "blue", "v2");
    if (scalar_input.value == 0) {
      console.error("Cannot divide by zero");
    } else {
      drawVector(vec1.div(scalar_input.value), "green", "v3");
      drawVector(vec2.div(scalar_input.value), "green", "v4");
    }
    ...
    drawVector(vec1, "red", "v1");
    drawVector(vec2, "blue", "v2");
    drawVector(vec1.mul(scalar_input.value), "green", "v3");
    drawVector(vec2.mul(scalar_input.value), "green", "v4");
    ...
    drawVector(vec1, "red", "v1");
    drawVector(vec2, "blue", "v2");
    drawVector(vec1.normalize(), "green", "v3");
    drawVector(vec2.normalize(), "green", "v4");
